...
class WGAN(object):
    def __init__(self,
        ...
        self.clip_updates = [w.assign(tf.clip_by_value(w, -self.c, self.c)) for w in d_vars]
        ...
        with self.sess.as_default():
            ...
            for epoch in range(EPOCH):
                for step in range(STEPS):
                    ...
                    # 训练D
                    for _ in range(1):
                        summary_str = self.sess.run(self.clip_updates, feed_dict={self.z: batch_z})
    ...
    def generator(self, z, train=True):
        ...
        with tf.variable_scope("generator") as scope:
            ...
            h1 = deconv2d(h0, [self.batch_size, s_h16, s_w16, self.gf_dim*8*16], k_h=1, k_w=1, s_h=1, s_w=1, scope='g_h1')
            h1 = tf.depth_to_space(h1, 4)

            h2 = deconv2d(h1, [self.batch_size, s_h4, s_w4, self.gf_dim*4*4],k_h=3,k_w=3, s_h=1, s_w=1, scope='g_h2')
            h2 = tf.depth_to_space(h2, 2)
            
            h3 = deconv2d(h2, [self.batch_size, s_h2, s_w2, self.c_dim*4],k_h=3,k_w=3, s_h=1, s_w=1, scope='g_h3')
